...
class MagicalGirlLevelOneDivOne {
    public:
    int t[2];
    string isReachable(vector<int> num, int x, int y) {
        string no = "NO", yes = "YES";
        clr (t, 0);
        for (int i : num) t[(i+1)&1] = 1;
        int tmp = (abs(x) + abs(y))&1;
        if ((tmp&1) && !t[tmp]) return no;
        return yes;;
    }
};
...
bool do_test(vector<int> jumpTypes, int x, int y, string __expected, int caseNo) {
...
bool run_testcase(int __no) {
    switch (__no) {
        case 0: {
            ...
            return do_test(to_vector(jumpTypes), x, y, __expected, __no);
        }
        case 1: {
            int jumpTypes[] = {
                3
