...
class AlienAndHamburgers
{
	public:
        int getNumber(vector <int> type, vector <int> taste){
            vec.clear(); tmp.clear();
            for (int i = 0; i < sz(type); ++ i) vec.pb (make_pair(type[i], taste[i]));
            sort(all(vec), cmp);
            int sum = 0, num = 0;
            for (int i = 0; i < sz(vec); ++ i){
                if (vec[i].second >= 0){
                    if (!i || vec[i].first != vec[i-1].first) ++ num;
                    sum += vec[i].second;
                }
                else if (!i || vec[i].first != vec[i-1].first) tmp.pb (vec[i]);
            }
            sort(all(tmp), cmp2);
            int ans = sum * num;
            for (int i = 0; i < sz(tmp); ++ i){
                ++ num; sum += tmp[i].second;
                int cnt = num * sum;
                if (cnt > ans) ans = cnt;
                else break;
            }
