...
#define lowbit(x) ((x)&(-x))
    ...
    void build (int _n, int _up, int *a){
        ...
        for (int i = 1; i <= n; ++ i){
            if (last[a[i]]){
                insert (last[a[i]], -num[a[i]], 0, up, x = rt0[i-1]);
                insert (i, num[a[i]], 0, up, rt0[i] = x);
            } else{
                insert (i, num[a[i]], 0, up, rt0[i] = rt0[i-1]);
            }
            last[a[i]] = i;
        }
        FG = 0;
    ...
    long long query (int ql, int qr, int l, int r, nod *x){
    ...
    long long solve (int l, int r){
        long long ret = query (l, r, 0, up, rt0[r]);
...
int main(){
    ...
    for (int i = 1, t; i <= n; ++ i){
        st[ t = std::lower_bound (s + 1, s + tot, a[i]) - s ].insert (i);
        num[t] = a[i]; a[i] = t;
    }
    for (int i = 0, t; i < m; ++ i) if (qq[i]){
        t = std::lower_bound (s + 1, s + tot, qr[i]) - s;
        num[t] = qr[i]; qr[i] = t;
    }
    ...
    for (int i = 0, l=ql[0], r=qr[0]; i < m; l = ql[++i], r = qr[i])
        if (qq[i]){
            ...
            T.modify (l, -num[a[l]], l, it == st[a[l]].end() ? n : (*it)-1);
            it1 = it--;
            if (it != st[a[l]].begin()){
                it--;
                T.modify (*it, num[a[l]], l, it1 == st[a[l]].end() ? n : (*it1)-1);
            }
            ...
            if (it != st[a[l]].begin()){
                it1--;
                T.modify (*it1, -num[a[l]], l, it == st[a[l]].end () ? n : (*it)-1);
            }
            T.modify (l, num[a[l]], l, it == st[a[l]].end () ? n : (*it)-1);
            st[a[l]].insert (l);
        } else{
