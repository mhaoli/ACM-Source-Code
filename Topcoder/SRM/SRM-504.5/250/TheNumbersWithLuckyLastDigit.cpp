...
class TheNumbersWithLuckyLastDigit {
    ...
    int find(int n) {
        int t = n % 10;
        repf (i, 0, 10) d[i] = mp (-1, -1);
        pii ans = solve (t);
        return ans.y <= n ? ans.x : -1;
    }
    pii solve (int t){
        if (d[t].x != -1) return d[t];
        if (t == 4) return mp (1, 4);
        if (t == 7) return mp (1, 7);

        pii ans = mp (inf, inf);
        repf (i, 0, 9) if (i == 4 || i == 7){
            int tmp = (t + 10 - i) % 10;
            if (tmp == t) continue;
            pii tp = solve (tmp);
            tp.x ++; tp.y += i;
...
int main(int argc, char *argv[]) {
    ...
    cout.precision(2);
    cout << "TheNumbersWithLuckyLastDigit (250 Points)" << endl << endl;
