...
class TheTicketsDivOne {
    ...
    double find(int n, int m) {
        ...
        repf (i, 2, n){
            d[i][1] = 0;
            double tmp = 1.0 / 2;
            repd (j, i-1, 1) d[i][1] += tmp * d[i-1][j], tmp /= 2;
            d[i][1] = (1.0/6 + d[i][1] * tre) / (1 - tmp);

            repf (j, 2, i) d[i][j] = d[i][j-1] / 2 + d[i-1][j-1] / 3;
        }
        return d[n][m];
    }
};
...
bool do_test(int n, int m, double __expected, int caseNo) {
    ...
    TheTicketsDivOne *instance = new TheTicketsDivOne();
...
bool run_testcase(int __no) {
    switch (__no) {
        case 0: {
            int n = 2;
            int m = 1;
            double __expected = 0.4444444444444444;
            return do_test(n, m, __expected, __no);
        }
        case 1: {
            int n = 2;
            int m = 2;
            double __expected = 0.5555555555555556;
            return do_test(n, m, __expected, __no);
        }
        case 2: {
            int n = 1;
        ...
        case 3: {
            int n = 3;
            int m = 2;
            double __expected = 0.31746031746031744;
            return do_test(n, m, __expected, __no);
        }
...
int main(int argc, char *argv[]) {
    ...
    cout.precision(2);
    cout << "TheTicketsDivOne (900 Points)" << endl << endl;
